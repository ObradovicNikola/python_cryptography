...
while recovered_message != original_message:
    ...
    if adjustment:
        padded_message = sslv3Pad(prefix +
                                  original_message[:-adjustment])
    else:
        padded_message = sslv3Pad(original_message)
    ...
    if success:
        ...
        print("Found one byte: {}. Took {} tries. Advancing to index {}".format(
            byte, one_byte_tries, index))
        one_byte_tries = 0
        # break
